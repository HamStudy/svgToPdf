...
import PDFKitBox = PDFKit.PDFKitBox;
import PDFKitFont = PDFKit.PDFKitFont;

type NodeSetting = SvgNode | 'none';

interface SvgToPdfImage {
  width: number;
  height: number;
}
interface SvgToPdfFont {
  font: PDFKitFont;
  size: number;
  fauxItalic: boolean;
  fauxBold: boolean;
}
type fontBaseline = 'middle' | 'central'
  | 'after-edge' | 'text-after-edge'
  | 'alphabetic' | 'auto' | 'baseline'
  | 'mathematical' | 'hanging'
  | 'before-edge' | 'text-before-edge'
  | '';
type fontShift = 'baseline' | 'sub' | 'super' | '';

type cssUnits = 'px' | 'pt' | 'cm' | 'mm' | 'in' | 'pc' | 'em' | 'ex' | '%' | '';

const allChildren = <const>['use', 'g', 'a', 'svg', 'image', 'rect', 'circle', 'ellipse', 'line', 'polyline', 'polygon', 'path', 'text'];
type allChildren = typeof allChildren[number];

type fontTextAnchor = 'start' | 'middle' | 'end';
type fontTextDirection = 'ltr' | 'rtl';

interface ArrayWithError<T> extends Array<T> {
  error?: string;
}

interface SvgToPdfCharacter {
  glyph: string;
  kern: number;
}
interface SvgToPdfTextPosition {
  glyph: string;
  unicode?: number[];
  rotate?: number;
  x?: number;
...
interface SvgToPdfSelector {
  tags: string[];
  ids: string[];
  classes: string[];
  specificity?: number;
}
interface SvgToPdfCssRule {
  selector: SvgToPdfSelector;
  css: Record<string, string>;
}

type LimitChildTypes<T extends {name: string}, K extends string> = T extends {name: K} ? T : never;
...
interface PDFDocumentEx extends PDFKit.PDFDocument {
  ...
  _ctm: Matrix;
  _doc: PDFKit.PDFKitReference;
}

export type SvgToPdfColor = [[number, number, number, number?], number];
export interface SvgToPdfOptions {
    ...
    documentCallback?: (
        file: string,
    ) => SvgNode | string | (SvgNode | string)[];

    // function called to get color, making mapping to CMYK possible
    colorCallback?: (color: SvgToPdfColor, raw?: string) => SvgToPdfColor;
...
class SvgNode {
  ...
  constructor(public tag: string, type: number, value: string, error?: any) {
    this.nodeName = tag;
...
type PathCommand = [string, boolean, boolean, ...number[]];

export function SVGtoPDF(_doc: PDFKit.PDFDocument, svg: string | SvgNode, x: number, y: number, options: SvgToPdfOptions) {
    const doc = _doc as PDFDocumentEx;

    const NamedColors: Record<string, [number, number, number]> = {aliceblue: [240,248,255], antiquewhite: [250,235,215], aqua: [0,255,255], aquamarine: [127,255,212], azure: [240,255,255], beige: [245,245,220], bisque: [255,228,196], black: [0,0,0], blanchedalmond: [255,235,205], blue: [0,0,255], blueviolet: [138,43,226], brown: [165,42,42], burlywood: [222,184,135], cadetblue: [95,158,160], chartreuse: [127,255,0],
    ...
    const DefaultColors: Record<string, SvgToPdfColor> = {black: [NamedColors.black, 1], white: [NamedColors.white, 1], transparent: [NamedColors.black, 0]};
    const Entities = <const>{quot: 34, amp: 38, lt: 60, gt: 62, apos: 39, OElig: 338, oelig: 339, Scaron: 352, scaron: 353, Yuml: 376, circ: 710, tilde: 732, ensp: 8194, emsp: 8195, thinsp: 8201, zwnj: 8204, zwj: 8205, lrm: 8206, rlm: 8207, ndash: 8211, mdash: 8212, lsquo: 8216, rsquo: 8217, sbquo: 8218, ldquo: 8220, rdquo: 8221, bdquo: 8222, dagger: 8224, Dagger: 8225, permil: 8240, lsaquo: 8249,
    ...
    const PathArguments = <const>{A: 7, a: 7, C: 6, c: 6, H: 1, h: 1, L: 2, l: 2, M: 2, m: 2, Q: 4, q: 4, S: 4, s: 4, T: 2, t: 2, V: 1, v: 1, Z: 0, z: 0};
    const PathFlags = <const>{A3: true, A4: true, a3: true, a4: true};
    const Properties = {
      'color':              {inherit: true, initial: undefined as string},
      'visibility':         {inherit: true, initial: 'visible', values: {'hidden': 'hidden', 'collapse': 'hidden', 'visible':'visible'}},
      'fill':               {inherit: true, initial: DefaultColors.black as SvgToPdfFillTypes},
      'stroke':             {inherit: true, initial: 'none' as SvgToPdfFillTypes},
      'stop-color':         {inherit: false, initial: DefaultColors.black as SvgToPdfColor | string},
      ...
      'stroke-dasharray':   {inherit: true, initial: [] as number[]},
      ...
      'text-anchor':        {inherit: true, initial: 'start' as fontTextAnchor, values: {'start':'start', 'middle':'middle', 'end':'end'}},
      'direction':          {inherit: true, initial: 'ltr' as fontTextDirection, values: {'ltr':'ltr', 'rtl':'rtl'}},
      'dominant-baseline':  {inherit: true, initial: 'baseline' as fontBaseline, values: {'auto':'baseline', 'baseline':'baseline', 'before-edge':'before-edge', 'text-before-edge':'before-edge', 'middle':'middle', 'central':'central', 'after-edge':'after-edge', 'text-after-edge':'after-edge', 'ideographic':'ideographic', 'alphabetic':'alphabetic', 'hanging':'hanging', 'mathematical':'mathematical'}},
      'alignment-baseline': {inherit: false, initial: <fontBaseline>undefined, values: {'auto':'baseline', 'baseline':'baseline', 'before-edge':'before-edge', 'text-before-edge':'before-edge', 'middle':'middle', 'central':'central', 'after-edge':'after-edge', 'text-after-edge':'after-edge', 'ideographic':'ideographic', 'alphabetic':'alphabetic', 'hanging':'hanging', 'mathematical':'mathematical'}},
      'baseline-shift':     {inherit: true, initial: 'baseline' as fontShift, values: {'baseline':'baseline', 'sub':'sub', 'super':'super'}},
      ...
      'marker-start':       {inherit: true, initial: 'none' as NodeSetting},
      'marker-mid':         {inherit: true, initial: 'none' as NodeSetting},
      'marker-end':         {inherit: true, initial: 'none' as NodeSetting},
      ...
      'clip-path':          {inherit: false, initial: 'none' as NodeSetting},
      'mask':               {inherit: false, initial: 'none' as NodeSetting},
    ...
    type PropertyKeyType<T extends string> = T extends keyof typeof Properties ? typeof Properties[T]['initial'] : any;

    class PDFGroup {
      name: string;
      resources: PDFKit.PDFKitReference;
      xobj: PDFKit.PDFKitReference;
      savedMatrix: Matrix;
      savedPage: PDFKit.PDFPage;
      page: {
        width: number;
        height: number;
        write(data: any): void;
        fonts: {[name: string]: PDFKit.PDFKitReference};
        xobjects: {[name: string]: PDFKit.PDFKitReference};
        ext_gstates: {[name: string]: PDFKit.PDFKitReference};
        patterns: {[name: string]: PDFKit.PDFKitReference};
      }
    }
    function docBeginGroup(bbox: PDFKitBox) {
      let group = new PDFGroup();
      ...
      doc.page = {
        ...
        write: function(data: any) {group.xobj.write(data);},
    ...
    function docEndGroup(group: PDFGroup) {
      ...
      group.resources.end(void 0);
      group.xobj.end(void 0);
      doc._ctm = group.savedMatrix;
      doc.page = group.savedPage;
    }
    function docInsertGroup(group: PDFGroup) {
      doc.page.xobjects[group.name] = group.xobj;
      doc.addContent('/' + group.name + ' Do');
    }
    function docApplyMask(group: PDFGroup, clip: boolean) {
    ...
    type SvgToPdfColorOutput = [PDFPattern | [number, number, number, number?] | PDFKit.PDFLinearGradient, number]
    type SvgToPdfColorOrPattern = [PDFPattern | [number, number, number, number?], number]
    type SvgToPdfFillTypes = SvgToPdfColorOrPattern | SvgElemGradient | SvgElemPattern | 'none';
    class PDFPattern {
      group: PDFGroup;
      dx: number;
      dy: number;
    ...
    function docCreatePattern(group: PDFGroup, dx: number, dy: number, matrix: Matrix) {
      let pattern = new PDFPattern();
      pattern.group = group;
      pattern.dx = dx;
      pattern.dy = dy;
    ...
    function docUsePattern(pattern: PDFPattern, stroke: boolean) {
    ...
    function isPattern(fill: any): fill is PDFPattern {
      return fill.constructor?.name === 'PDFPattern' || fill instanceof PDFPattern;
    }
    function docFillColor(color: SvgToPdfColorOrPattern) {
      const cVal = color[0];
      if (isPattern(cVal)) {
        doc.fillOpacity(color[1]);
        docUsePattern(cVal, false);
      } else {
        doc.fillColor(cVal as any, color[1]);
      }
    }
    function docStrokeColor(color: SvgToPdfColorOrPattern) {
      const cVal = color[0];
      if (isPattern(cVal)) {
        doc.strokeOpacity(color[1]);
        docUsePattern(cVal, true);
      } else {
        doc.strokeColor(cVal as any, color[1]);
      }
    }
    ...
    function docApplyDash(dashArray: number[], dashOffset: number) {
      let index;
      ...
      // TODO: Verify that this is really valid, since the PDFKit documentation says it's not =]
      doc.dash(<any>dashArray, {phase: dashOffset});
    }
    function docInsertLink(x: number, y: number, w: number, h: number, url: string) {
      let ref = doc.ref({
        Type: 'Annot',
        Subtype: 'Link',
        Rect: [x, y, w, h],
    ...
    function parseXml(xml: string) {
      ...
      function recursive() {
        ...
        if (temp = parser.match(/^<([\w:.-]+)\s*/, true)) { // Opening tag
          ...
          return new SvgNode(null, 8, <string>temp, error);
        } else if (temp = parser.match(/^<\?[\s\S]*?\?>/)) { // Processing instructions
          return new SvgNode(null, 7, <string>temp, error);
        } else if (temp = parser.match(/^<!DOCTYPE\s*([\s\S]*?)>/)) { // Doctype
          return new SvgNode(null, 10, <string>temp, error);
    ...
    function decodeEntities(str: string) {
      return(str.replace(/&(?:#([0-9]+)|#[xX]([0-9A-Fa-f]+)|([0-9A-Za-z]+));/g, function(mt, m0: string, m1: string, m2: keyof typeof Entities) {
    ...
    function parseColor(raw: string) {
      let temp: string | RegExpMatchArray, result: SvgToPdfColor;
      let fndColor = NamedColors[(<keyof typeof NamedColors>raw)];
      raw = (raw || '').trim();
      if (fndColor) {
        result = [[...fndColor], 1];
      } else if (temp = raw.match(/^cmyk\(\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9.]+)\s*\)$/i)) {
        // CMYK
        const parsed = [ temp[1], temp[2], temp[3] ].map(v => parseInt(v, 10)) as SvgToPdfColor[0];
        parsed.push(parseFloat(temp[4]));
        if (parsed.every(v => v <= 100)) {
          result = [parsed, 1];
        }
      } else if (temp = raw.match(/^rgba\(\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9.]+)\s*\)$/i)) {
        // RGBA
        const parsed = [
          ...[ temp[1], temp[2], temp[3] ].map(v => parseInt(v, 10)) as SvgToPdfColor[0],
          parseFloat(temp[4]),
        ];
        if (parsed.slice(0, 3).every(v => v <= 255) && parsed[3] <= 1) {
        ...
        // RBG
        const parsed = [ temp[1], temp[2], temp[3] ].map(v => parseInt(v, 10)) as SvgToPdfColor[0];
        if (parsed.every(v => v < 256)) {
          result = [parsed, 1];
        }
      } else if (temp = raw.match(/^rgb\(\s*([0-9.]+)%\s*,\s*([0-9.]+)%\s*,\s*([0-9.]+)%\s*\)$/i)) {
        const parsed = [ temp[1], temp[2], temp[3] ].map(v => parseFloat(v) * 2.55) as SvgToPdfColor[0];
    ...
    function opacityToColor(color: SvgToPdfColor, opacity: number, isMask: boolean = false): SvgToPdfColor {
      let [newColor, newOpacity] = color;
      newOpacity *= opacity;
      // let newColor = [...color[0]],
      //     newOpacity = color[1] * opacity;
      if (isMask) {
        newColor = newColor.map(c => c * newOpacity) as SvgToPdfColor[0];
        return [newColor, 1];
      } else {
        return [[...newColor], newOpacity];
      }
    }
    function multiplyMatrix(...args: Matrix[]) {
    ...
    function transformPoint(p: [number, number], m: Matrix) {
    ...
    function validateMatrix(m: Matrix): Matrix {
    ...
    function solveEquation(curve: number[]) {
    ...
    function getCurveValue(t: number, curve: number[]) {
      return (curve[0] || 0) + (curve[1] || 0) * t + (curve[2] || 0) * t * t + (curve[3] || 0) * t * t * t;
    }
    function isEqual(number: number, ref: number) {
      return Math.abs(number - ref) < 1e-10;
    }
    function isNotEqual(number: number, ref: number) {
      return Math.abs(number - ref) >= 1e-10;
    }
    function validateNumber(n: number) {
      return n > -1e21 && n < 1e21 ? Math.round(n * 1e6) / 1e6 : 0;
    }
    function isArrayLike<T extends unknown[]>(v: any): v is T {
      return typeof v === 'object' && v !== null && typeof v.length === 'number';
    }
    function parseTranform(v: string) {
      let parser = new StringParser((v || '').trim());
      let result: Matrix = [1, 0, 0, 1, 0, 0];
      let temp: string | RegExpMatchArray;
    ...
    function parseAspectRatio(aspectRatio: string, availWidth: number, availHeight: number, elemWidth: number, elemHeight: number, initAlign = 0): Matrix {
    ...
    function parseStyleAttr(vIn: string) {
      let result: Record<string, string> = Object.create(null);
      const v = (vIn || '').trim().split(/;/);
    ...
    function parseSelector(v: string) {
    ...
    function parseStyleSheet(v: string) {
    ...
    function matchesSelector(elem: SvgNode, selector: SvgToPdfSelector) {
    ...
    function getStyle(elem: SvgNode) {
      let result: CSSStyleDeclaration = Object.create(null);
    ...
    function combineArrays<T extends unknown>(array1: T[], array2: T[]): T[] {
      return array1.concat(array2.slice(array1.length));
    }
    function getAscent(font: PDFKitFont, size: number) {
      return Math.max(font.ascender, (font.bbox[3] || font.bbox.maxY) * (font.scale || 1)) * size / 1000;
    }
    function getDescent(font: PDFKitFont, size: number) {
      return Math.min(font.descender, (font.bbox[1] || font.bbox.minY) * (font.scale || 1)) * size / 1000;
    }
    function getXHeight(font: PDFKitFont, size: number) {
      return (font.xHeight || 0.5 * (font.ascender - font.descender)) * size / 1000;
    }
    function getBaseline(font: PDFKitFont, size: number, baseline: fontBaseline, shift: fontShift | number) {
      let dy1, dy2;
      switch (baseline) {
      ...
      switch (shift) {
        ...
        default: dy2 = shift as number; break;
      }
      return dy1 - dy2;
    }
    function getTextPos(font: PDFKitFont, size: number, text: string) {
      let [hex, pos] = font.encode('' + text);
      let data: SvgToPdfTextPosition[] = [];
    ...
    type SvgElementTypeInheritable = never
      | SvgElemUse | SvgElemSymbol | SvgElemGroup | SvgElemLink
      | SvgElemSvg | SVGElemImage | SvgElemRect | SvgElemCircle
      | SvgElemEllipse | SvgElemLine | SvgElemPolyline | SvgElemPolygon
      | SvgElemPath | SvgElemText | SvgElemTspan | SvgElemTextPath;
    type SvgElementType = never
      | SvgElemUse | SvgElemSymbol | SvgElemGroup | SvgElemLink
      | SvgElemSvg | SVGElemImage | SvgElemRect | SvgElemCircle
      | SvgElemEllipse | SvgElemLine | SvgElemPolyline | SvgElemPolygon
      | SvgElemPath | SvgElemText | SvgElemTspan | SvgElemTextPath | SvgElemTextNode
      | SvgElem;
    function createSVGElement(obj: SvgNode, inherits: SvgElementTypeInheritable): SvgElementType {
      switch (obj.nodeName) {
        ...
        case 'a': return makeSvgElemLink(obj, inherits);
    ...
    class StringParser {
      ...
      match(exp: string | RegExp, all?: false): string;
      match(exp: string | RegExp, all: true): RegExpMatchArray;
      match(exp: string | RegExp, all = false) {
      ...
      matchLengthUnit() {
        return this.match(/^(?:px|pt|cm|mm|in|pc|em|ex|%|)/) as cssUnits;
      }
      matchNumber() {
    ...
    class BezierSegment {
      ...
      #divisions: number;
      constructor(public p1x: number, public p1y: number, public c1x: number, public c1y: number, public c2x: number, public c2y: number, public p2x: number, public p2y: number) {
        let divisions = this.#divisions = 6 * precision;
        let equationX = this.#equationX = [p1x, -3 * p1x + 3 * c1x, 3 * p1x - 6 * c1x + 3 * c2x, -p1x + 3 * c1x - 3 * c2x + p2x];
        let equationY = this.#equationY = [p1y, -3 * p1y + 3 * c1y, 3 * p1y - 6 * c1y + 3 * c2y, -p1y + 3 * c1y - 3 * c2y + p2y];
        let derivativeX = this.#derivativeX = [-3 * p1x + 3 * c1x, 6 * p1x - 12 * c1x + 6 * c2x, -3 * p1x + 9 * c1x - 9 * c2x + 3 * p2x];
        let derivativeY = this.#derivativeY = [-3 * p1y + 3 * c1y, 6 * p1y - 12 * c1y + 6 * c2y, -3 * p1y + 9 * c1y - 9 * c2y + 3 * p2y];
        let lengthMap: number[] = this.#lengthMap = [0];
        for (let i = 1; i <= divisions; i++) {
          let t = (i - 0.5) / divisions;
          let dx = getCurveValue(t, derivativeX) / divisions,
              dy = getCurveValue(t, derivativeY) / divisions,
              l = Math.sqrt(dx * dx + dy * dy);
          lengthMap.push(lengthMap[i - 1] + l);
        }
      ...
      getBoundingBox(): PDFKitBox {
        let temp: number;
        let minX = getCurveValue(0, this.#equationX), minY = getCurveValue(0, this.#equationY),
            maxX = getCurveValue(1, this.#equationX), maxY = getCurveValue(1, this.#equationY);
        if (minX > maxX) {temp = maxX; maxX = minX; minX = temp;}
        if (minY > maxY) {temp = maxY; maxY = minY; minY = temp;}
        let rootsX = solveEquation(this.#derivativeX);
        for (let i = 0; i < rootsX.length; i++) {
          if (rootsX[i] >= 0 && rootsX[i] <= 1) {
            let x = getCurveValue(rootsX[i], this.#equationX);
            if (x < minX) {minX = x;}
            if (x > maxX) {maxX = x;}
          }
        }
        let rootsY = solveEquation(this.#derivativeY);
        for (let i = 0; i < rootsY.length; i++) {
          if (rootsY[i] >= 0 && rootsY[i] <= 1) {
            let y = getCurveValue(rootsY[i], this.#equationY);
            if (y < minY) {minY = y;}
            if (y > maxY) {maxY = y;}
          }
        }
      ...
      getPointAtLength(l: number) {
        ...
        for (let i = 1; i <= this.#divisions; i++) {
          let l1 = this.#lengthMap[i-1], l2 = this.#lengthMap[i];
          if (l1 <= l && l <= l2) {
            let t = (i - (l2 - l) / (l2 - l1)) / this.#divisions,
                x = getCurveValue(t, this.#equationX), y = getCurveValue(t, this.#equationY),
                dx = getCurveValue(t, this.#derivativeX), dy = getCurveValue(t, this.#derivativeY);
    ...
    class LineSegment {
      ...
      constructor(public p1x: number, public p1y: number, public p2x: number, public p2y: number) {
    ...
    type SvgShapeSegment = LineSegment | BezierSegment;
    class SvgShape {
      pathCommands: PathCommand[] = [];
      pathSegments: SvgShapeSegment[] = [];
      startPoint: [number, number, number];
      endPoint: [number, number, number];
      totalLength: number;
      ...
      move(x: number, y: number): null {
        this.#startX = this.#currX = x; this.#startY = this.#currY = y;
        return null;
      }
      line(x: number, y: number): LineSegment {
        const segment = new LineSegment(this.#currX, this.#currY, x, y);
        this.#currX = x; this.#currY = y;
        return segment;
      }
      curve(c1x: number, c1y: number, c2x: number, c2y: number, x: number, y: number) {
        const segment = new BezierSegment(this.#currX, this.#currY, c1x, c1y, c2x, c2y, x, y);
        this.#currX = x; this.#currY = y;
        return segment;
      }

      close() {
        let segment = new LineSegment(this.#currX, this.#currY, this.#startX, this.#startY);
        this.#currX = this.#startX; this.#currY = this.#startY;
        return segment;
      }
      addCommand(data: PathCommand) {
        this.pathCommands.push(data);
        let segment: SvgShapeSegment = this[data[0] as 'close'].apply(this, data.slice(3));
        if (segment) {
      ...
      C(c1x: number, c1y: number, c2x: number, c2y: number, x: number, y: number) {
        ...
        this.#lastCom = 'C'; this.#lastCtrlX = c2x; this.#lastCtrlY = c2y;
        return this;
      }
      c(c1x: number, c1y: number, c2x: number, c2y: number, x: number, y: number) {
        return this.C(this.#currX + c1x, this.#currY + c1y, this.#currX + c2x, this.#currY + c2y, this.#currX + x, this.#currY + y);
      }
      S(c1x: number, c1y: number, x: number, y: number) {
        return this.C(this.#currX + (this.#lastCom === 'C' ? this.#currX - this.#lastCtrlX : 0), this.#currY + (this.#lastCom === 'C' ? this.#currY - this.#lastCtrlY : 0), c1x, c1y, x, y);
      }
      s(c1x: number, c1y: number, x: number, y: number) {
        return this.C(this.#currX + (this.#lastCom === 'C' ? this.#currX - this.#lastCtrlX : 0), this.#currY + (this.#lastCom === 'C' ? this.#currY - this.#lastCtrlY : 0), this.#currX + c1x, this.#currY + c1y, this.#currX + x, this.#currY + y);
      }
      Q(cx: number, cy: number, x: number, y: number) {
        let c1x = this.#currX + 2 / 3 * (cx - this.#currX), c1y = this.#currY + 2 / 3 * (cy - this.#currY),
            c2x = x + 2 / 3 * (cx - x), c2y = y + 2 / 3 * (cy - y);
        this.addCommand(['curve', true, true, c1x, c1y, c2x, c2y, x, y]);
        this.#lastCom = 'Q'; this.#lastCtrlX = cx; this.#lastCtrlY = cy;
        return this;
      }
      q(c1x: number, c1y: number, x: number, y: number) {
        return this.Q(this.#currX + c1x, this.#currY + c1y, this.#currX + x, this.#currY + y);
      }
      T(x: number, y: number) {
        return this.Q(this.#currX + (this.#lastCom === 'Q' ? this.#currX - this.#lastCtrlX : 0), this.#currY + (this.#lastCom === 'Q' ? this.#currY - this.#lastCtrlY : 0), x, y);
      }
      t(x: number, y: number) {
        return this.Q(this.#currX + (this.#lastCom === 'Q' ? this.#currX - this.#lastCtrlX : 0), this.#currY + (this.#lastCom === 'Q' ? this.#currY - this.#lastCtrlY : 0), this.#currX + x, this.#currY + y);
      }
      A(rx: number, ry: number, fi: number, fa: number, fs: number, x: number, y: number) {
        if (isEqual(rx, 0) || isEqual(ry, 0)) {
          ...
          fa = 1 * (fa ? 1 : 0);
          fs = 1 * (fs ? 1 : 0);
          let x1 = Math.cos(fi) * (this.#currX - x) / 2 + Math.sin(fi) * (this.#currY - y) / 2,
              y1 = Math.cos(fi) * (this.#currY - y) / 2 - Math.sin(fi) * (this.#currX - x) / 2,
          ...
          let cx = Math.cos(fi) * x2 - Math.sin(fi) * y2 + (this.#currX + x) / 2,
              cy = Math.sin(fi) * x2 + Math.cos(fi) * y2 + (this.#currY + y) / 2,
      ...
      path(d: string) {
        ...
        while (command = parser.match(/^[astvzqmhlcASTVZQMHLC]/)) {
          if (typeof command !== 'string') { throw new Error("Unexpected value type"); }
          parser.matchSeparator();
          let values = [];
          while (value = (PathFlags[<'A3'>(<string>command + values.length)] ? parser.match(/^[01]/) : parser.matchNumber())) {
            parser.matchSeparator();
            if (values.length === PathArguments[command as 'A']) {
              this[<'a'>command].apply(this, values);
              values = [];
          ...
          if (values.length === PathArguments[command as 'A']) {
            this[command as 'A'].apply(this, values);
          } else {
      ...
      getBoundingBox() {
        let bbox: PDFKitBox = [Infinity, Infinity, -Infinity, -Infinity];
        function addBounds(bbox1: PDFKitBox) {
      ...
      transform(m: Matrix) {
        ...
        for (let i = 0; i < this.pathCommands.length; i++) {
          ...
          const [a, b, c, ...points] = data;
          for (let j = 0; j < points.length; j += 2) {
            [points[j], points[j+1]] = transformPoint([points[j], points[j + 1]], m)
          }
          this.addCommand([a, b, c, ...points]);
        }
        return this;
      }
      mergeShape(shape: SvgShape) {
        for (let i = 0; i < shape.pathCommands.length; i++) {
          this.addCommand([...shape.pathCommands[i]]);
        }
        return this;
      }
      clone() {
      ...
      insertInDocument() {
        for (let i = 0; i < this.pathCommands.length; i++) {
          let [command, , , ...values] = this.pathCommands[i];
          switch(command) {
      ...
      getMarkers() {
        let markers: number[][] = [], subPaths = this.getSubPaths();
    ...
    class SvgElem {
      ...
      stack: SvgNode[];
      inherits: SvgElementTypeInheritable;
      ...
      #childrenCache: SvgElementType[] = null;

      constructor(protected obj: SvgNode, inherits: SvgElementTypeInheritable)  {
        this.name = obj.nodeName;
        this.isOuterElement = obj === svg || !obj.parentNode;
        this.inherits = inherits || (!this.isOuterElement ? createSVGElement(obj.parentNode, null) as SvgElementTypeInheritable : null);
        this.stack = (this.inherits ? this.inherits.stack.concat(obj) : [obj]);
        this.style = parseStyleAttr(typeof obj.getAttribute === 'function' && obj.getAttribute('style'));
        this.css = useCSS ? getComputedStyle(obj as any) : getStyle(obj);
      }
      ...
      resolveUrl(value: string) {
        ...
        if (id) {
          if (!file) {
            let svgObj = (<SvgNode>svg).getElementById(id);
            if (svgObj) {
          ...
          if (documentCallback) {
            ...
            if (!svgs) {
              ...
              for (let i = 0; i < svgs.length; i++) {
                const cur = svgs[i];
                if (typeof cur === 'string') {svgs[i] = parseXml(cur); }
              }
              documentCache[file] = svgs;
            }
            svgs = Array.isArray(svgs) ? svgs : [svgs];
            for (let i = 0; i < svgs.length; i++) {
              const cur = svgs[i];
              if (typeof cur === 'string') throw new Error("SVGtoPDF: svg doc cache is a string");
              let svgObj = cur.getElementById(id);
              if (svgObj) {
                if (this.stack.indexOf(svgObj) === -1) {
                  return svgObj;
                } else {
      ...
      computeUnits(valIn: string | number, unit: cssUnits, percent: number, isFontSize: boolean = false) {
        const value = parseFloat(valIn as string);
        if (unit === '%') {
          return value / 100 * (isFontSize || percent != null ? percent : this.getViewport());
      ...
      computeLength<I extends number | string>(value: string, percent: number, initial?: I, isFontSize: boolean = false): number | I {
        let parser = new StringParser((value || '').trim()), temp1: string, temp2: cssUnits;
      ...
      computeLengthList(value: string, percent: number, strict: boolean = false) {
      ...
      getLength<I extends number | string>(key: string, percent: number, initial?: I): number | I {
        return this.computeLength(this.attr(key), percent, initial);
      }
      getLengthList(key: string, percent: number) {
        return this.computeLengthList(this.attr(key), percent);
      }
      getUrl(key: string) {
        return this.resolveUrl(this.attr(key))
      }
      getNumberList(key: string) {
        let parser = new StringParser((this.attr(key) || '').trim()), result: ArrayWithError<number> = [], temp;
      ...
      getViewbox(key: string, initial: PDFKitBox): PDFKitBox {
        let viewBox = this.getNumberList(key);
        if (viewBox.length === 4 && viewBox[2] >= 0 && viewBox[3] >= 0) { return viewBox as PDFKitBox; }
        return initial;
      }
      getPercent(key: string, initial: number) {
        let value = this.attr(key);
        let parser = new StringParser((value || '').trim()), temp1, temp2;
        let number = parseFloat(parser.matchNumber());
        if (!number) {return initial;}
        if (parser.match('%')) { number *= 0.01; }
        if (parser.matchAll()) {return initial;}
        return Math.max(0, Math.min(1, number));
      }
      chooseValue<T extends any>(...args: T[]) : T {
        for (let i = 0; i < args.length; i++) {
          if (args[i] && args[i] === args[i]) { return args[i]; }
        }
        return args[args.length - 1];
      }

      get<T extends string>(key: T): PropertyKeyType<T> {
        if (this.#styleCache[key] !== undefined) {return this.#styleCache[key];}
        let keyInfo = Properties[key as keyof typeof Properties] || (<any>{}), value: any, result: any;
        for (let i = 0; i < 3; i++) {
          switch (i) {
            case 0:
          ...
          if (value === 'inherit') {
            ...
            if (result != null) {return this.#styleCache[key] = result;}
          }
        ...
        return this.#styleCache[key] = (keyInfo.inherit && this.inherits ? this.inherits.get(key) : keyInfo.initial);
      }

      getChildren(): SvgElementType[] {
        if (this.#childrenCache) { return this.#childrenCache; }
        let children = [];
        for (let i = 0; i < this.obj.childNodes.length; i++) {
          let child = this.obj.childNodes[i];
          if (!child.error && this.allowedChildren.indexOf(child.nodeName) !== -1) {
            children.push(createSVGElement(child, this as any));
          }
        }
      ...
      // These are on some children but not all
      getBoundingShape?: () => SvgShape;
      drawInDocument?: (isClip?: boolean, isMask?: boolean) => void;
    }

    class SvgElemStylable extends SvgElem {
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
      }
      ...
      clip() {
        const cPath = this.get('clip-path');
        if (cPath !== 'none') {
          let clipPath = new SvgElemClipPath(cPath, null);
          clipPath.useMask((clipPath.attr('clipPathUnits') === 'objectBoundingBox') ? this.getBoundingBox() : null);
          return true;
        }
      }
      mask() {
        const cMask = this.get('mask');
        if (cMask !== 'none') {
          let mask = new SvgElemMask(cMask, null);
          mask.useMask(this.getBoundingBox());
          return true;
        }
      }
      getFill(isClip: boolean, isMask: boolean): SvgToPdfColor{
        ...
        if (fill !== 'none' && opacity && fillOpacity) {
          if (fill instanceof SvgElemGradient || fill instanceof SvgElemPattern) {
            return fill.getPaint(this.getBoundingBox(), fillOpacity * opacity, isClip, isMask) as any;
          }
          return opacityToColor(fill as SvgToPdfColor, fillOpacity * opacity, isMask);
        }
      }
      getStroke(isClip: boolean, isMask: boolean): SvgToPdfColor {
        let opacity = this.get('opacity'),
        ...
        if (stroke !== 'none' && opacity && strokeOpacity) {
          if (stroke instanceof SvgElemGradient || stroke instanceof SvgElemPattern) {
            return stroke.getPaint(this.getBoundingBox(), strokeOpacity * opacity, isClip, isMask) as any;
          }
          return opacityToColor(stroke as SvgToPdfColor, strokeOpacity * opacity, isMask);
        }
      }
    }

    class SvgElemHasChildren extends SvgElemStylable {
      allowedChildren: string[] = [...allChildren];
      
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
      }
      getBoundingShape = function getBoundingShape() : SvgShape {
        let shape = new SvgShape(),
            children = this.getChildren();
        for (let i = 0; i < children.length; i++) {
          const curChild = children[i];
          if (curChild.get('display') !== 'none') {
            if (typeof curChild.getBoundingShape === 'function') {
              let childShape = children[i].getBoundingShape().clone();
              if ('getTransformation' in curChild && typeof curChild.getTransformation === 'function') {
                childShape.transform(curChild.getTransformation());
              }
      ...
      getChildren() {
        return super.getChildren() as LimitChildTypes<SvgElementType, allChildren>[];
      }
      drawChildren(isClip: boolean, isMask: boolean) {
        let children = this.getChildren();
        for (let i = 0; i < children.length; i++) {
          const child = children[i];
          if (child.get('display') !== 'none') {
            if ('drawInDocument' in child) {
              (<SvgElem>child).drawInDocument(isClip, isMask);
            }
          }
        }
      }
    ...
    class SvgElemContainer extends SvgElemHasChildren {
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
      }

      drawContent(isClip: boolean, isMask: boolean) {
        this.transform();
    ...
    class SvgElemUse extends SvgElemContainer {
      name: 'use';
      #child: ReturnType<SvgElemHasChildren['getChildren']>[0];
      #x: number;
      #y: number;
      ...
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        ...
        let child = this.getUrl('href') || this.getUrl('xlink:href');
        if (child) { this.#child = createSVGElement(child, this) as any; }
      }

      getChildren() {
        const outList = [] as ReturnType<SvgElemHasChildren['getChildren']>;
        if (this.#child) outList.push(this.#child);
        return outList;
      }
      drawInDocument = function(this: SvgElemUse, isClip: boolean = false, isMask: boolean = false) {
        doc.save();
        this.drawContent(isClip, isMask);
        doc.restore();
      }
      getTransformation = function() {
        return multiplyMatrix(this.get('transform'), [1, 0, 0, 1, this.#x, this.#y]);
      };
    }
    let a: SvgElemSymbol;
    class SvgElemSymbol extends SvgElemContainer {
      name: 'symbol';
      #aspectRatio: string;
      #viewBox: PDFKitBox;
      ...
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        ...
        this.#viewBox = this.getViewbox('viewBox', [0, 0, width, height]);
      }
      getVWidth() {
      ...
      drawInDocument = function(this: SvgElemSymbol, isClip: boolean = false, isMask: boolean = false) {
        doc.save();
        this.drawContent(isClip, isMask);
        doc.restore();
      }
      getTransformation = function(this: SvgElemSymbol) {
        const viewBox = this.#viewBox;
        return multiplyMatrix(parseAspectRatio(this.#aspectRatio, this.#width, this.#height, viewBox[2], viewBox[3]), [1, 0, 0, 1, -viewBox[0], -viewBox[1]]);
      }
    }

    class SvgElemGroup extends SvgElemContainer {
      name: 'g' | 'a';
      
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
      }
      drawInDocument = function(this: SvgElemGroup | SvgElemLinkGroup, isClip: boolean = false, isMask: boolean = false) {
        doc.save();
        if ('link' in this && !isClip && !isMask) { this.addLink();}
        this.drawContent(isClip, isMask);
        doc.restore();
      }
      getTransformation = function(this: SvgElemGroup) {
        return this.get('transform');
      }
    }
    
    interface SvgElemLink extends SvgElem {
      name: 'a';
      href: string;
      target: string;
      link: string;
    ...
    const SvgElemLinkMixin = {
      construct(this: SvgElemLink) {
        this.href = this.get('href');
        this.target = this.get('target');
        this.link = this.attr('href') || this.attr('xlink:href');
      },
      addLink(this: SvgElemLink) {
    ...
    class SvgElemLinkGroup extends SvgElemGroup implements SvgElemLink {
      name: 'a';
      href: string;
      target: string;
      link: string;

      allowedChildren = ['textPath', 'tspan', '#text', '#cdata-section', 'a'];

      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
        SvgElemLinkMixin.construct.call(this);
      }
      readonly addLink = SvgElemLinkMixin.addLink;
    }
    
    class SvgElemSvg extends SvgElemContainer {
      name: 'svg';
      #x: number;
      #y: number;
      #width: number;
      ...
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        ...
        if (this.isOuterElement && preserveAspectRatio) {
          ...
          this.#aspectRatio = preserveAspectRatio;
        }
      }
      getVWidth() {
        return this.#viewBox[2];
      ...
      drawInDocument = function(this: SvgElemSvg, isClip: boolean = false, isMask: boolean = false) {
      ...
      getTransformation = function(this: SvgElemSvg) {
        const [aspectRatio, width, height, viewBox] = [this.#aspectRatio, this.#width, this.#height, this.#viewBox];
    ...
    class SVGElemImage extends SvgElemStylable {
      name: 'image';
      #x: number;
      #y: number;
      #width: number;
      ...
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
        const link = imageCallback(this.attr('href') || this.attr('xlink:href') || '');
        this.#x = this.getLength('x', this.getVWidth(), 0);
        this.#y = this.getLength('y', this.getVHeight(), 0);
        let width = this.getLength('width', this.getVWidth(), 'auto');
        let height = this.getLength('height', this.getVHeight(), 'auto');
        // this.#image;
        try {
          this.#image = doc.openImage(link);
        } catch(e) {
      ...
      getTransformation = function(this: SVGElemImage) {
        return this.get('transform');
      }
      getBoundingShape = function(this: SVGElemImage) {
        const [width, height] = [this.#width, this.#height];
        return new SvgShape().M(x, y).L(x + width, y).M(x + width, y + height).L(x, y + height);
      }
      drawInDocument = function(this: SVGElemImage, isClip: boolean = false, isMask: boolean = false) {
        const [width, height, image] = [this.#width, this.#height, this.#image];
        ...
        if (!isClip) {
          ...
          // TODO: Verify what this should do, because it definitely isn't valid
          docFillColor(DefaultColors.white); //.fill();
        }
        doc.restore();
      }
    ...
    class SvgElemPattern extends SvgElemHasChildren {
      ref: SvgElemPattern;

      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable, private fallback: SvgToPdfColor) {
        super(obj, inherits);
        this.ref = (function() {
      ...
      attr(key: string): ReturnType<SvgElemHasChildren['attr']> {
        let attr = super.attr(key);
        if (attr != null || key === 'href' || key === 'xlink:href') { return attr; }
        return this.ref ? this.ref.attr(key) : null;
      }
      getChildren(): ReturnType<SvgElemHasChildren['getChildren']> {
        let children = super.getChildren();
        if (children.length > 0) {return children;}
        return this.ref ? this.ref.getChildren() : [];
      }
      getPaint(bBox: PDFKitBox, gOpacity: number, isClip = false, isMask = false) {
    ...
    class SvgElemGradient extends SvgElem {
      ref: SvgElemGradient;
      allowedChildren = ['stop'];
      ...
      getChildren(): SvgElem[] {
        let children = super.getChildren();
        if (children.length > 0) { return children as SvgElem[]; }
        return this.ref ? this.ref.getChildren() : [];
      }
      getPaint(bBox: PDFKitBox, gOpacity: number, isClip = false, isMask = false): SvgToPdfColorOutput {
        let children = this.getChildren();
        ...
        if (children.length === 1) {
          ...
          return opacityToColor(stopColor as SvgToPdfColor, child.get('stop-opacity') * gOpacity, isMask);
        }
        ...
        if (matrix = validateMatrix(matrix)) {
          ...
          for (let n = 0; n < nTotal; n++) {
            ...
            for (let i = 0; i < children.length; i++) {
              ...
              stopColor = opacityToColor(stopColor as SvgToPdfColor, child.get('stop-opacity') * gOpacity, isMask);
              offset = Math.max(offset, inOrder ? child.getPercent('offset', 0) : 1 - child.getPercent('offset', 0));
              if (i === 0 && stopColor[0].length === 4) { (<any>grad)._colorSpace = 'DeviceCMYK'; } // Fix until PR #763 is merged into PDFKit
              if (i === 0 && offset > 0) {
                grad.stop((n + 0) / nTotal, <any>stopColor[0], stopColor[1]);
              }
              grad.stop((n + offset) / (nAfter + nBefore + 1), <any>stopColor[0], stopColor[1]);
              if (i === children.length - 1 && offset < 1) {
                grad.stop((n + 1) / nTotal, <any>stopColor[0], stopColor[1]);
              }
            }
          }
    ...
    class SvgElemBasicShape extends SvgElemStylable {
      shape: SvgShape;
      dashScale = 1;
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
      }
      getBoundingShape = function(this: SvgElemBasicShape) {
        return this.shape;
      }
      getTransformation = function(this: SvgElemBasicShape) {
        return this.get('transform');
      }
      drawInDocument = function(this: SvgElemBasicShape, isClip: boolean = false, isMask: boolean = false) {
    ...
    class SvgElemRect extends SvgElemBasicShape {
      name: 'rect';
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
        let x = this.getLength('x', this.getVWidth(), 0),
            ...
            rx = this.getLength<number>('rx', this.getVWidth()),
            ry = this.getLength<number>('ry', this.getVHeight());
    ...
    class SvgElemCircle extends SvgElemBasicShape {
      name: 'circle';
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
    ...
    class SvgElemEllipse extends SvgElemBasicShape {
      name: 'ellipse';
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
        let cx = this.getLength('cx', this.getVWidth(), 0),
    ...
    class SvgElemLine extends SvgElemBasicShape {
      name: 'line';
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
    ...
    class SvgElemPolyline extends SvgElemBasicShape {
      name: 'polyline';
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
    ...
    class SvgElemPolygon extends SvgElemBasicShape {
      name: 'polygon';
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
        let points = this.getNumberList('points');
    ...
    class SvgElemPath extends SvgElemBasicShape {
      name: 'path';
      pathLength: number;
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
        this.shape = new SvgShape().path(this.attr('d'));
        let pathLength = this.getLength('pathLength', this.getViewport()) as number;
        this.pathLength = pathLength > 0 ? pathLength : undefined;
        this.dashScale = (this.pathLength !== undefined ? this.shape.totalLength / this.pathLength : 1);
      }
    }
    class SvgElemMarker extends SvgElemHasChildren {
      #width: number;
      #height: number;
      #viewBox: PDFKitBox;
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
        let width = this.#width = this.getLength('markerWidth', this.getParentVWidth(), 3);
        let height = this.#height = this.getLength('markerHeight', this.getParentVHeight(), 3);
        this.#viewBox = this.getViewbox('viewBox', [0, 0, width, height]);
      }
      getVWidth() {
        return this.#viewBox[2];
      ...
      drawMarker(isClip: boolean, isMask: boolean, posArray: number[], strokeWidth: number) {
        const [width, height, viewBox] = [this.#width, this.#height, this.#viewBox];
        doc.save();
    ...
    class SvgElemClipPath extends SvgElemHasChildren {
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
      }
      useMask(bBox: PDFKitBox) {
    ...
    class SvgElemMask extends SvgElemHasChildren {
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
      }
      useMask(bBox: PDFKitBox) {
    ...
    class SvgElemTextContainer extends SvgElemStylable {
      _font: SvgToPdfFont;
      _pos: SvgToPdfTextPosition[];
      ...
      allowedChildren = ['tspan', '#text', '#cdata-section', 'a'];
      isText = true;
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
      }
      getChildren(): (SvgElemTextTypes | SvgElemTextNode)[] {
        return super.getChildren() as SvgElemTextTypes[];
      }
      getBoundingShape = function(this: SvgElemTextContainer) {
        let shape = new SvgShape();
      ...
      drawTextInDocument(this: SvgElemTextContainer | SvgElemLinkTspan, isClip: boolean = false, isMask: boolean = false) {
        if ('link' in this && !isClip && !isMask) { this.addLink(); }
          ...
          switch(childElem.name) {
            case 'tspan': case 'textPath': case 'a':
              if (childElem.get('display') !== 'none') {
                (<SvgElemTspan>childElem).drawTextInDocument(isClip, isMask);
              }
              break;
      ...
      decorate(lineWidth: number, linePosition: number, isClip: boolean = false, isMask: boolean = false) {
        let fill = this.getFill(isClip, isMask),
            stroke = this.getStroke(isClip, isMask);
        if (fill) {
          docFillColor(fill);
    ...
    class SvgElemTextNode {
      name: '#text' | '#cdata-section';
      textContent: string;
      ...
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        this.name = obj.nodeName as '#text' | '#cdata-section';
        this.textContent = obj.nodeValue;
      }
    }
    class SvgElemTspan extends SvgElemTextContainer {
      name: 'tspan' | 'a';
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
      }
    }
    class SvgElemLinkTspan extends SvgElemTspan implements SvgElemLink {
      name: 'a';
      href: string;
      target: string;
      link: string;
    ...
    function makeSvgElemLink(obj: SvgNode, inherits: SvgElementTypeInheritable): SvgElemLink {
      if (inherits && 'isText' in inherits) {
        return new SvgElemLinkTspan(obj, inherits);
      } else {
        return new SvgElemLinkGroup(obj, inherits);
      }
    }

    class SvgElemTextPath extends SvgElemTextContainer {
      name: 'textPath';
      pathObject: SvgShape;
      pathLength: number;
      pathScale: number;
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        ...
        if ((temp = this.attr('path')) && temp.trim() !== '') {
          let pathLength = this.getLength<number>('pathLength', this.getViewport());
    ...
    type SvgElemTextTypes = SvgElemText | SvgElemTspan | SvgElemTextPath | SvgElemLinkTspan;
    class SvgElemText extends SvgElemTextContainer {
      name: 'text';
      textContent: string;
      ...
      constructor(obj: SvgNode, inherits: SvgElementTypeInheritable) {
        super(obj, inherits);
        this.processTextElem(this);
      }
      
      private processTextElem(textParentElem: SvgElemTextTypes) {
        let processedText = '';
        let remainingText = this.obj.textContent;
        let textPaths: SvgElemTextPath[] = [];
        let currentChunk: SvgToPdfTextPosition[] = [];
        let currentAnchor: fontTextAnchor;
        let currentDirection: fontTextDirection;
        let currentX = 0;
        let currentY = 0;
        ...
        function adjustLength(pos: SvgToPdfTextPosition[], length: number, spacingAndGlyphs: boolean) {
        ...
        function recursive(currentElem: SvgElemTextTypes, parentElem: SvgElemTextTypes) {
              ...
              case '#text': case '#cdata-section':
                ...
                for (let w = 0; w < words.length; w++) {
                  ...
                  for (let j = 0; j < pos.length; j++) {
                    ...
                    let position: SvgToPdfTextPosition = {
                      glyph: pos[j].glyph,
        ...
        function textOnPath(currentElem: SvgElemTextPath) {
          let pathObject = currentElem.pathObject,
      ...
      getTransformation = function(this: SvgElemText) {
        return this.get('transform');
      }

      drawInDocument = function(this: SvgElemText, isClip: boolean = false, isMask: boolean = false) {
        doc.save();
        this.transform();
        this.clip();
    ...
    var pxToPt = options.assumePt ? 1 : 72 / 96, // 1px = 72/96pt, but only if assumePt is false
      ...
      groupStack: PDFGroup[] = [],
      documentCache: Record<string, ReturnType<typeof documentCallback>> = {},
      links: PDFKit.PDFKitReference[] = [],
      styleRules: SvgToPdfCssRule[] = [];
    ...
    if (typeof colorCallback !== 'function') {
      ...
      for (let k in DefaultColors) {
        const color: keyof typeof DefaultColors = k as any;
        let newColor = colorCallback(DefaultColors[color] as SvgToPdfColor);
